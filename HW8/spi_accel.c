...
void accel_read_print(){
    volatile unsigned char bufferaccel [100];
   ...
   //Temporary Variables for Reading ACCEL
    ...
    display_clear();
    OLED_WRITE(0,0, pbufferaccel);
//    OLED_WRITE(0,0, (unsigned char *) bufferaccel);
}

void accel_read_snake(){
   volatile unsigned char buffaccsnake [100];
   ...
   //Temporary Variables for Reading ACCEL
    ...
    acc_read_register(TEMP_OUT_L, (unsigned char *) &tempt, 2);
    sprintf(buffaccsnake,"%d)",accels[0],accels[1]);
    display_clear();
    OLED_SNAKE((unsigned char *) pbuffaccsnake);
    //OLED_SNAKE((unsigned char *) buffaccsnake);
}